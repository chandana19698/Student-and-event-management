...
if($name=='')
{
	echo "<script>alert('No student found with the given ID $ids' )</script>";
}
else
{
    ...
    if(isset($_POST['update']))
    {
		...
		$course =$_POST['course'];
		  ...
		  $stmt = $conn->prepare("UPDATE STUDENTS SET STUD_NAME=?, STUD_DOB=?, STUD_PHONE=?, STUD_IC=?,STUD_COURSE=? where Stud_id=?");
		  $stmt->execute(array($name,$dob,$phone,$ic,$course,$id));
		  
      echo "<script>alert('Data updated And old data has been added to Students_audit table by Triggers!!.')</script>";
    }
