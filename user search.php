...
if(isset($_GET["id"]))
{
	...
	try
	{
		...
		//select data from db
		$stmt = $conn->prepare("SELECT count(*) as count,STUD_ID, STUD_NAME, DATE_FORMAT(STUD_DOB,'%d-%m-%Y') AS DATE,
		...
		if($cnt < 1)
		{
			echo "<script>alert('No student found with the given ID $id' )</script>";
		}
